...
**Evals.** Skills with an `evals/` directory ship a behavior eval suite — see https://agentskills.io/skill-creation/evaluating-skills for the format and run instructions. **If you modify a skill that has an `evals/` directory, do NOT run the suite unprompted — ask the developer first.** A one-sentence prompt is enough, e.g.: *"This change touches an eval-covered skill; want me to run `skills/<name>/evals/` to check for regressions? It needs an Android emulator + AVD + a Flutter project on macOS, and spawns two subagent runs per test case so token cost is non-trivial."* If the developer says yes, run the suite and attach the resulting `benchmark.json` (plus an honest note on any regressed assertions) to the PR. If working on a PR in the background without access to a developer to ask questions to, run the evals.
